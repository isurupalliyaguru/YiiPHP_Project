...
class Navigation extends CApplicationComponent
{
	...
	public function getSubMenuItems($menuHTML, $menuItemHTML, $menuLinkHTML, $options_arr) {
		/*
		pre	: called in the view file.
		@param $options_arr - can be empty, see getSpecifiedMenus()
		...
		*/
		...
		if (!empty($this_pageid)) {
			...
			if (!empty($row_sub_menus))
				$row_sub_menus_items['r'] = PagemenuItems::model()->getDbResult_SubMenuItems($row_sub_menus['menuid']);
			else return false;
			$menuArr = array('result'=>'r', 'dataset'=>$row_sub_menus_items);
			$sub_navigation = $this->getSpecifiedMenus ($menuArr, $menuHTML, $menuItemHTML, $menuLinkHTML, $options_arr, $this_pageid, $this_parentpageid=false);
			return $sub_navigation['r'];
		}
		else return false;
	}
	...
	public function getSpecifiedMenus ($menuArr, $menuHTML, $menuItemHTML, $menuLinkHTML, $options_arr='', $this_pageid='', $this_parentpageid=true){
		/* 	
		pre:	@param $menuArr - string with menu special id text (t1, t2 etc)
				...
				@param $this_pageid - passes the page ID, could be empty
				@param $this_parentpageid - passes the parent page ID, default true. Could be false when a situation like quering sub menu
		...
		*/
		...
		if(isset($menuArr['result'])){
			$Allmenu = $menuArr['dataset'];
			unset($menuArr['dataset']);
		}
		else{
